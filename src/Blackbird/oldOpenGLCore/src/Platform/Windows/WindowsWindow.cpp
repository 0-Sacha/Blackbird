...
#include "OpenGLCore/Renderer/Context/RendererContext.h"
...
namespace OGLC {
	...
	static void GLFWErrorCallback(int error, const char* description) {
		OGLC_FATAL("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		OGLC_INFO("Creating Windows-window {0}  #{{1}, {2}}", props.Title, props.Width, props.Height);


		if (!s_GLFWInitialized) {
			int success = glfwInit();
			OGLC_ASSERT(success, "Could not initialize GLFW!");
		...
		m_RendererContext = new RendererContext(m_Window);
